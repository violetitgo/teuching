...
<link rel="stylesheet" href="http://52.78.116.59:8080/teuching/resources/css/message/msgBoxRecv.css">
</head>
<body>
				...
				<table>
					...
					<tbody id="tbody">
						<c:forEach items="${res.msg }" var="msg" varStatus="cnt">
							<form id="form"
								action="http://52.78.116.59:8080/teuching/message/msgrecvdetail.do"
								target="pop">
								<tr>
					...
					<div class="block-27">
						<ul>
							<c:if test="${res.paging.blockStart le 1 }">
								<li><a
									href="http://52.78.116.59:8080/teuching/message/msgboxrecv.do?currentpage=${res.paging.blockStart}">&lt;</a></li>
							</c:if>
							<c:if test="${res.paging.blockStart gt 1 }">
								<li><a
									href="http://52.78.116.59:8080/teuching/message/msgboxrecv.do?currentpage=${res.paging.blockStart-1}">&lt;</a></li>
							</c:if>
							...
							<c:forEach begin="${res.paging.blockStart }"
								...
								<c:if test="${res.paging.currentPage eq p }">
									<li class="active"><a
										href="http://52.78.116.59:8080/teuching/message/msgboxrecv.do?currentpage=${p}">${p }</a></li>
								</c:if>
							...
							<c:if test="${res.paging.blockEnd lt res.paging.lastPage }">
								<li><a
									href="http://52.78.116.59:8080/teuching/message/msgboxrecv.do?currentpage=${res.paging.blockEnd+1}">&gt;</a></li>
							</c:if>
							<c:if test="${res.paging.blockEnd ge res.paging.lastPage }">
								<li><a
									href="http://52.78.116.59:8080/teuching/message/msgboxrecv.do?currentpage=${res.paging.blockEnd}">&gt;</a></li>
							</c:if>
						</ul>
					</div>
				</div>
